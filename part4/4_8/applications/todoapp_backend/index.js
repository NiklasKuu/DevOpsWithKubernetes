...
app.get('/backend/todos', cors(), async (req, res) => {
  const todos = (await db_todo.findAll()).map( x => { return { text: x.text, done: x.done, id: x.id  }} );
...
app.post('/backend/todos', cors(), async (req, res) => {
  ...
  connect({ servers: process.env.NATS_URL }).then(async (nc) => {
    const sc = StringCodec();
    nc.publish("messages", sc.encode("A todo was created"));
...
app.put('/backend/todos/:id', async (req, res) => {
  ...
  const prevDone = await db_todo.findByPk(id);

  const updateCount = await db_todo.update({ done: !prevDone.done }, {
    where: {
      id: prevDone.id
    }
  });
  if(updateCount == 1) {
    ...
    connect({ servers: process.env.NATS_URL }).then(async (nc) => {
        const sc = StringCodec();
        nc.publish("messages", sc.encode("A todo was updated"));  

    })

    res.send('todo updated');
  } else {
    res.send('could not update todo');
  }
});
...
app.get('/', (req,res) => {
  sequelize.authenticate().then( () => res.status(200).send()).catch(() => res.status(500).send());
})
//for some reason this needs to be here, otherwise ingress wont allow requests to the backend, because it thinks its down
